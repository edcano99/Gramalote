...
datosrutas = $('#datosrutas').DataTable({ 
 ...
 "columns":[
     ...
     "render": function ( data, type, fullI, meta ) {
            var idgeocerca = fullI.idgeocerca;

          // var capacitacion = fullI.nom_capacitacion;
         return "<div class='text-center'><div class='btn-group'><button class='btn btn-link bi-trash btnBorrar ' value='"+idgeocerca+"' id =''>Eliminar</button></div></div>";
     }
          },
 ],
...
$(document).on("click", ".btnBorrar", function(){
    fila = $(this);
    //console.log('borrar');
    idgeocerca = this.value;		
    opcion = 3; //eliminar 
    $.ajax({
           url: "?P=geocerca",
           type: "POST",
           datatype:"json",    
           data:  {opcion:opcion, idgeocerca:idgeocerca},    
           success: function() {
